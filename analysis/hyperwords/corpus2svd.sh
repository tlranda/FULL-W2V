...
# Parse input params
PARAM_CHECK=$(python2 hyperwords/corpus2svd_params.py $@ 2>&1)
...
mkdir $OUTPUT_DIR
python2 hyperwords/corpus2pairs.py $CORPUS2PAIRS_OPTS $CORPUS.clean > $OUTPUT_DIR/pairs
scripts/pairs2counts.sh $OUTPUT_DIR/pairs > $OUTPUT_DIR/counts
python2 hyperwords/counts2vocab.py $OUTPUT_DIR/counts


# Calculate PMI matrices for each collection of pairs
python2 hyperwords/counts2pmi.py $COUNTS2PMI_OPTS $OUTPUT_DIR/counts $OUTPUT_DIR/pmi


# Create embeddings with SVD
python2 hyperwords/pmi2svd.py $PMI2SVD_OPTS $OUTPUT_DIR/pmi $OUTPUT_DIR/svd
...
python2 hyperwords/svd2text.py $SVD2TEXT_OPTS $OUTPUT_DIR/svd $OUTPUT_DIR/vectors.txt
