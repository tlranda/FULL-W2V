...
# Parse input params
PARAM_CHECK=$(python2 hyperwords/corpus2sgns_params.py $@ 2>&1)
...
python2 hyperwords/text2numpy.py $OUTPUT_DIR/sgns.words
python2 hyperwords/text2numpy.py $OUTPUT_DIR/sgns.contexts


# Save the embeddings in the textual format 
python2 hyperwords/sgns2text.py $SGNS2TEXT_OPTS $OUTPUT_DIR/sgns $OUTPUT_DIR/vectors.txt
